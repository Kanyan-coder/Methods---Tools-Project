...
class Inventory:
    ...
    def getPrice(self, isbn):
        self.cursor.execute("SELECT Price FROM {} WHERE isbn = ?".format(self.tableName), (isbn,))
        result = self.cursor.fetchone()
        if result:
            return result[0]
        else:
    ...
    def getBook(self, isbn):
            self.cursor.execute("SELECT ISBN, Name, Price FROM {} WHERE ISBN = ?".format(self.tableName), (isbn,))
            result = self.cursor.fetchall()
            return result
